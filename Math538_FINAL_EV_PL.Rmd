...
* $r_t$ is the dependent variable, representing the log return of the AAPL stock on day t.  It is calculated as the difference in the logarithm of the closing prices between two consecutive days, i.e., log($close_t$) - log($close_{t-1}$).

* $r_{t-1}$ is the independent variable, which is the log return of the AAPL stock on the previous day t-1.
...
hist_gg <- function(data, x_var, title = "Histogram", x_axis_label = "X Axis") {
...
New variables 'return' and 'lagged_return' are calculated using the `dplyr` package in the code chunk below. These represent the daily returns of the stock and the previous day's return, respectively. `The na.omit` function removes any rows with missing values which can't be used in the subsequent regression analysis.

```{r,  lagged_return_calc }

stocks2 <- stocks

# Calculate 'return' and 'lagged_return'
stocks <- stocks %>%
  mutate(
...
imputed <- mice(data = stocks, m = 10, maxit = 30, method = 'pmm', seed = 555)
...
hist_gg(post_samples, b_Intercept, "Posterior of Intercept", expression(beta ~ 'Intercept'))
```
...
hist_gg(post_samples, b_lagged_return, "Posterior of Lagged Return", 'Lagged Return')
```
...
hist_gg(post_samples, sigma, expression('Posterior of' ~ sigma), expression(sigma ~ 'Posterior'))
```
