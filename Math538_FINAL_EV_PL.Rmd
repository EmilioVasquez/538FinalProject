...
The PMM method is a standalone, non-Bayesian imputation method. The imputed datasets generated are then used further using Bayesian methods in this project.  By creating multiple imputations (m = 5), `mice` acknowledges and represents the uncertainty inherent in the imputation process.
...
$$\textbf{Figure X.}\text{ Histogram of the difference of the actual close price vs the imputed data points}  $$
Figure X shows the difference from the original score vs the imputed score for all 5 multiply imputed datasets combined into one visualiztion. As one can see, the vast majority of points had a very small difference between the actual and the imputed. There were a few outliers that had imputed a value greater than \$20 but overall the imputation was a success. The appendix hosts histograms of differences for all 5 data sets individually.
...
$$\textbf{Figure X.}\text{ Imputed results for dataset 3 overlayed the actual close price }  $$
...
### Talk to nik about...
#I noticed we only take the posterior samples for the first dataset, but would it better illustrate our point of multiple imputation if we take the posterior samples for all 5 fits
# of brms?

post_samples <- as.data.frame(rbind(posterior_samples(fits[[1]], pars = c("Intercept", "lagged_close", "sigma")),
...
The below chunk will calculate a MAP estimate as well as 95\% credible interval estimates for the parameters $\beta, \beta_0$.
...
\begin{tabular}{||c c c c||} 
 ...
 \hline
 $\sigma$ & 88 & 2.508397 & 2.622535\\ [1ex] 
 \hline
\end{tabular}
\end{center}
...
density_gg(post_samples, b_Intercept, "Posterior of Intercept", expression(beta ~ 'Intercept')) +
  geom_vline(xintercept = combined_results$ci_lower[1], color = "red", linetype = 'dotted') +
  geom_vline(xintercept = combined_results$ci_upper[1], color = "red", linetype = 'dotted') +
  geom_vline(xintercept = combined_results$Estimate[1],color = "green", linetype = 'dotted')
```



```{r, intercept_samples_efv, echo = FALSE}
# Histogram for the Intercept
#hist(post_samples$b_Intercept, breaks = 30, main = "Posterior of #Intercept", 
#     xlab = "Intercept", col = "blue")

post_plot <- density_gg(post_samples, b_Intercept, "Posterior of Intercept", expression(beta ~ 'Intercept')) 

map <- mode_density(post_plot)
ci_low <- quantile(post_samples$b_Intercept, 0.025)
ci_high <- quantile(post_samples$b_Intercept, 0.975)
...
```{r, lagged_return_samples_efv, echo = FALSE}
...
# Histogram for sigma
#hist(post_samples$sigma, breaks = 30, main = "Posterior of Sigma", xlab = "Sigma", col = "red")

density_gg(post_samples, sigma, expression('Posterior of' ~ sigma), expression(sigma ~ 'Posterior')) + geom_vline(xintercept = combined_results$ci_lower[1]) + geom_vline(xintercept = combined_results$ci_upper[1]) + 
  geom_vline(xintercept = combined_results$Estimate[1])
```



```{r, sigma_post_samples_efv, echo = FALSE}
plot <- density_gg(post_samples, sigma, expression('Posterior Distribution of' ~ sigma), expression(sigma ~ 'Posterior')) 
map <- mode_density(plot)
ci_low <- quantile(post_samples$sigma, 0.025)
ci_high <- quantile(post_samples$sigma, 0.975)
...
summary(fit_ols)
```
$$\textbf{Figure X.}\text{ OLS parameter estimates using complete data}  $$


From the figure above, one can see that the intercept and the coefficient are close to the multiply imputed Bayesian regression results.
...
```{r}
hist_gg(stocks, close_preamp - imp1, title = "Histogram of Difference of Original Data and Imputed Data (Imputed Data 1", x_axis_label = "Difference", binwidth = .5)

hist_gg(stocks, close_preamp - imp2, title = "Histogram of Difference of Original Data and Imputed Data (Imputed Data 2", x_axis_label = "Difference", binwidth = .5)

hist_gg(stocks, close_preamp - imp3, title = "Histogram of Difference of Original Data and Imputed Data (Imputed Data 3", x_axis_label = "Difference", binwidth = .5)

hist_gg(stocks, close_preamp - imp4, title = "Histogram of Difference of Original Data and Imputed Data (Imputed Data 4", x_axis_label = "Difference", binwidth = .5)

hist_gg(stocks, close_preamp - imp5, title = "Histogram of Difference of Original Data and Imputed Data (Imputed Data 1", x_axis_label = "Difference")
```
...
```{r, bayesian_regression_model_fit_appendix, results = 'hide', message = FALSE, warning = FALSE}
# Initialize an empty list to store the models
fits <- vector("list", length = 5)
