...
\doublespacing
{\bf Missing Data - Multiple Imputation Methods }
...
With removing entire rows of data, this might be an excellent and easy strategy to implement if dealing with a large dataset. Removing 2,000 rows of data from 2,000,000 rows will not affect the analysis or inference all that much if at all.  Working with a small dataset (think 100 or less rows), throwing out incomplete data could result in losing valuable information and bring potential biases into any estimates provided to a stakeholder. The next approach is to apply the expectation maximization (EM) algorithm where one replaces NAs with the conditional average. The details on a third method now follow.
...
Performing a data amputation simulation like this allows one to verify how well one's imputation performs under different assumptions of missing data.
...
The `mice` function is used to impute missing values in the dataset. It uses Predictive Mean Matching ('pmm') method to fill in missing 'close' prices by default. Here it is specified in the interest of readability. Multiple imputed datasets are generated (specified by m = 5) to account for the uncertainty of the imputation process. Lastly, the 5 imputed datasets are extracted using the 'complete' function from within 'mice'.
